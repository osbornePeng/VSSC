%% This code do the parameters estimation test for spile-slab share and spike-slab, Lasso pool groups together for simulation.
...
%                 type(j).trainset_x=randn(type(j).n,d);
            ...
            %% learning
            trainset_x=[type(1).trainset_x;type(2).trainset_x;type(3).trainset_x];
            trainset_y=[type(1).trainset_y;type(2).trainset_y;type(3).trainset_y];
            
            for j=1:3
                
                %% training for normal spike_slab
                if (j==1)
                    [sigma_e,sigma_b,a_p,a_2,mu_2,s_b,l_bound_2]=spike_slab(trainset_x,trainset_y);
                end
                ass_2(:,j)=a_2;
                muss_2(:,j)=mu_2;
                w2=a_2().*mu_2();
                ...
                tic
                if (j==1)
                    fitinfo=cvglmnet(trainset_x,trainset_y,[],options);
                end
                ...
                tic
                if (j==1)
                    fitinfo=cvglmnet(trainset_x,trainset_y);
                end
